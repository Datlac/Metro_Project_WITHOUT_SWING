...
public class FareCalculator {
    ...
    // --- MA TRẬN GIÁ VÉ (14x14) ---
    // Hàng/Cột tương ứng với S01 -> S14 (Index 0 -> 13)
    ...
    private static void initFareMatrix() {
        ...
        // =========================================================================
        // NHẬP LIỆU CHÍNH XÁC TỪ FILE "SỬA GIÁ.TXT"
        // =========================================================================
        
        // 1. Từ Bến Thành (S01 - Index 0)
        setPriceRow(0, 
            6000,  // Đến S2, S3, S4, S5, S6, S7 (Zone 1)
            new int[]{1, 2, 3, 4, 5, 6} 
        );
        ...
        setPriceRow(3, 
            6000, // Đến S1, S2, S3, S5, S6, S7, S8, S9
        ...
        updatePrice(8, 1, 8000);   // Đến Nhà Hát (Ghi đè cho khớp file: 9k trong file ghi nhầm, nhưng logic trên là 8k cho S2->S9? File ghi S9->S1,S2=9k)
        ...
        // File ghi: "từ khu công nghệ cao đi zone1(15.000đ){bến thành, nhà hát thành phố}". Vậy S12->S2 là 15k.
        ...
        // File đoạn cuối ghi "từ khu công nghệ cao đi zone1(19.000đ)" khả năng cao là typo của "Suối Tiên" vì giá cao nhất.
    ...
    private static int getStationIndex(String rawName) {
        ...
        // 1. Metro Stations
        if (lower.contains("ben thanh") || lower.contains("s01")) return 0;
        if (lower.contains("nha hat") || lower.contains("s02")) return 1;
        if (lower.contains("ba son") || lower.contains("s03")) return 2;
        if (lower.contains("van thanh") || lower.contains("s04")) return 3;
        if (lower.contains("tan cang") || lower.contains("s05")) return 4;
        if (lower.contains("thao dien") || lower.contains("s06")) return 5;
        if (lower.contains("an phu") || lower.contains("s07")) return 6;
        if (lower.contains("rach chiec") || lower.contains("s08")) return 7;
        if (lower.contains("phuoc long") || lower.contains("s09")) return 8;
        if (lower.contains("binh thai") || lower.contains("s10")) return 9;
        if (lower.contains("thu duc") && !lower.contains("ky thuat") || lower.contains("s11")) return 10;
        if (lower.contains("cong nghe cao") || lower.contains("s12")) return 11;
        if (lower.contains("quoc gia") || lower.contains("s13")) return 12;
        if (lower.contains("suoi tien") && !lower.contains("bus") || lower.contains("s14")) return 13;

        // 2. Bus Stations (Map về ga Metro gần nhất để tính giá tương đối)
        if (lower.contains("nong lam")) return 13; // Gần Suối Tiên
        if (lower.contains("su pham (q5)")) return 0; // Gần Bến Thành
        if (lower.contains("su pham ky thuat")) return 10; // Gần Thủ Đức
